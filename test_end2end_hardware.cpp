...
int main(int argc, char** argv){
    ...
    for (int i = 0; i < 6; i++) {
        ...
        // Group the events (the first event is the leader)
    ...
    // Enable performance counter（the leader）
    ...
    // Disable performance counter (the leader)
    ioctl(fd[0], PERF_EVENT_IOC_DISABLE, PERF_IOC_FLAG_GROUP);

    // Output the performance counting results
    for (int i = 0; i < 6; i++) {
